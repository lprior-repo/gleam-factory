...
pub fn apply_fix_errors_when_no_code_blocks_test() {
  stage_runner.apply_fix("Just some text without code", "/tmp/test", domain.Gleam)
  |> should.be_error
}
...
pub fn code_block_with_multiline_content_test() {
  ...
  let block = stage_runner.CodeBlock(filename: None, content: content, lang: "gleam")
  case block {
...
pub fn can_discriminate_success_from_failure_test() {
  ...
  let failure = stage_runner.StageFailed(reason: "oops", iterations: 1, tokens_used: 50)
