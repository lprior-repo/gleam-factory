...
pub fn agent_success_captures_output_test() {
  let result =
    agent_executor.AgentSuccess(
      output: "Implementation complete",
      artifacts: [],
    )
  case result {
    agent_executor.AgentSuccess(output: output, ..) ->
      output |> should.equal("Implementation complete")
  }
}

pub fn agent_success_captures_artifacts_test() {
  let result =
    agent_executor.AgentSuccess(output: "done", artifacts: [
      "main.gleam",
      "test.gleam",
    ])
  case result {
    agent_executor.AgentSuccess(artifacts: artifacts, ..) ->
      artifacts |> should.equal(["main.gleam", "test.gleam"])
  }
}
...
pub fn agent_failure_captures_reason_test() {
  ...
  case result {
    agent_executor.AgentFailure(reason: reason) ->
      reason |> should.equal("tests failed")
  }
}
...
pub fn execution_context_captures_worktree_path_test() {
  ...
  case ctx {
    agent_executor.ExecutionContext(worktree_path: value, ..) ->
      value |> should.equal("/workspace/my-task")
  }
}
...
pub fn execution_context_captures_task_spec_test() {
  ...
  case ctx {
    agent_executor.ExecutionContext(task_spec: value, ..) ->
      value |> should.equal("Implement the login feature with OAuth")
  }
}
