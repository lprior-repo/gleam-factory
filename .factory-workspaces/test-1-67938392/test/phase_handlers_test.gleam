...
pub fn implementer_can_write_src_test() {
  ...
  let result =
    phase_handlers.handle_write_file(
      "src/test_file.gleam",
      "test",
      llm.Implementer,
    )
...
pub fn rejects_path_escape_test() {
  let result =
    phase_handlers.handle_write_file("../escape.txt", "test", llm.Implementer)
  should.be_error(result)
}

pub fn rejects_absolute_path_test() {
  let result =
    phase_handlers.handle_write_file("/etc/passwd", "test", llm.Implementer)
  should.be_error(result)
}
