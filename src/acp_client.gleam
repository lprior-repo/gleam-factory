...
pub fn handle_create_session(state: AcpClientState) -> Result(#(String, AcpClientState), String) {
...
pub fn handle_send_prompt(
  ...
  use cmd_result <- result.try(process.run_command("curl", ["-s", "-X", "POST", "-H", "Content-Type: application/json", "-d", json_body, types.get_base_url(client)], ""))
  use response <- result.try(case cmd_result {
    process.Success(stdout, _, 0) -> Ok(stdout)
    process.Success(_, _, code) | process.Failure(_, code) -> Error("http failed " <> string.inspect(code))
  })
...
fn parse_prompt_response(json_str: String) -> Result(#(String, List(types.AcpNotification)), String) {
  use notifs <- result.try(extract_notifications(json_str))
  use content <- result.try(extract_content(json_str))
  Ok(#(content, notifs))
}

fn extract_notifications(json_str: String) -> Result(List(types.AcpNotification), String) {
...
fn extract_content(json_str: String) -> Result(String, String) {
  ...
  json.parse(json_str, decoder) |> result.map_error(fn(_) { "invalid response" })
}

fn store_notifications(store: types.UpdateStore, notifs: List(types.AcpNotification)) -> types.UpdateStore {
  case notifs {
    [] -> store
